...
public class ProductTourActivity extends AppCompatActivity {
    //首先定义它的数量五张图片5个WelcomeFragment
    static final int NUM_PAGES = 5;
    //这里是包含viewpager以及里面的button控件的适配
    ViewPager pager;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //创建一个五张滑动引导页开启的入口activity_tutorial
        ...
        //当然有了跳过肯定会有一个下一张或者说用户下一个执行什么
        //这个的时候要find找到一个id也就是下一张还是一样的OnClick点击事件
        next = ImageButton.class.cast(findViewById(R.id.next));
        next.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                //这里的onClick时间每点击一次pager加一次
                //等到达五次那么就结束
                pager.setCurrentItem(pager.getCurrentItem() + 1, true);
            }
        });
        //结束完五张之后需要find一个id——done也就是完成还是一样的onClick事件
